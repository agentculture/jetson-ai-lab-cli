...
- URLs are extracted from four sources, all deduped against each other: message.content by regex; attachments\[\].url; embeds\[\].url; and the bodies of embeds — description and field values — by the same regex. The fourth source exists because the t1 probe measured that link/article embeds carry a url duplicating one already in content, while rich embeds carry url=None and hide their links in description and fields. None of these are available today — `_serialize_message` (jlab/cli/`_discord.py`:202-211) reads none of them — so the extractor depends on the seam extension in c18.
...
- Embed extraction covers embeds\[\].url AND the regex-extracted URLs in embed description and field values, with every source deduped against content-derived URLs so an auto-preview embed does not double-count a link already in the message text. The t1 probe measured both halves of the problem: type=link and type=article embeds carry a url that duplicates content, and type=rich embeds carry url=None with their links in the body.
  - instruction: Either drop embeds\[\].url and rely on content plus attachments, or extract from embed description/fields as well and dedupe against content-derived URLs. Do not ship embeds\[\].url alone — it is the worst of both.
  - honesty: A URL that appears both in message content and in an auto-generated preview embed produces exactly one link record, and a URL that exists only inside a rich embed's description or fields is extracted.
- Attachment URLs are kept but presented as EPHEMERAL, with the jump link as the durable pointer. The t1 probe measured them expiring 14-22 hours from fetch, independent of message age, because the signature is minted at fetch time — so the report must say on its face that an attachment URL expires within about a day rather than presenting it as a stable link. Re-running the scan mints fresh signatures, which is what makes the cache in c40 a partial answer only: a cache-rendered report's attachment URLs are as stale as the cache.
  - instruction: Mark attachment URLs as expiring in both the HTML and the CSV, and render the jump link beside every one. A cache-rendered report must not present its cached attachment URLs as live.
  - honesty: An attachment URL in the report is visibly marked as expiring, with a jump link beside it, in both the HTML and the CSV — and a report rendered from a cache older than the expiry window says so rather than presenting dead URLs as live.
...
- VERIFIED by the t1 probe against the live guild (2026-09-05): Discord attachment CDN URLs ARE signed and expiring. Every sampled attachment on cdn.discordapp.com carried ex/is/hm parameters and expired 14-22 hours out — and crucially the expiry is minted at FETCH time, not from the message: a 143-day-old message's attachment URL expired in 14.7h, the same window as a 1.6-day-old one. An attachment URL written into a report is therefore dead within a day, while looking perfectly valid. The durable pointer is the jump link, which the probe confirmed is present on every message.
...
- `s22` — `challenge pass / t1 probe: live guild attachment + embed sampling (read-only, 100 public text channels, 146 messages)`: Attachment URLs are signed with ex/is/hm and expire 14-22h from FETCH, independent of message age (143d-old message, 14.7h expiry). `jump_url` is present on every message sampled. Embed types observed: link and article carry a url that duplicates content; rich carries url=None.
  - seeds: `c36`, `c41`
