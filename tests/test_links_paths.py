...
def test_reports_dir_rejects_a_symlinked_ancestor(
    ...
    must be refused rather than silently followed, or scraped Discord URLs
    and author ids would land outside the gitignored boundary.
    """
    ...
    monkeypatch.setattr(links_paths, "find_repo_root", lambda: fake_root)
    ...
    assert not (outside / "links").exists()
...
def test_reports_dir_rejects_being_itself_a_symlink(
    ...
    (fake_root / "data" / "reports" / "links").symlink_to(outside, target_is_directory=True)
...
def test_run_dir_rejects_a_preexisting_symlinked_run_id(
    ...
    outside.mkdir()
    reports_dir = fake_root / "data" / "reports" / "links"
    ...
    with pytest.raises(CliError) as exc:
        links_paths.links_run_dir("hostile-run")
    assert exc.value.code == EXIT_ENV_ERROR
    assert exc.value.remediation
