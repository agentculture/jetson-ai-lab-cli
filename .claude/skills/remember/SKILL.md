...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope devague
...
scope:** the default personal scope is **private** (`--scope devague
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `devague`). Private records are served only to a recall in the same
