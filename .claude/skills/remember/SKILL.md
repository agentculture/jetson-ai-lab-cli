...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope jetson-ai-lab-cli
...
scope:** the default personal scope is **private** (`--scope jetson-ai-lab-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `jetson-ai-lab-cli`). Private records are served only to a recall in the same
