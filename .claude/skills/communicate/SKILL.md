...
type: command
description: >
  Cross-repo + mesh communication from jetson-ai-lab-cli: file tracked GitHub issues
  ...
  the next step lives outside jetson-ai-lab-cli (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- jetson-ai-lab-cli (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-jetson-ai-lab-cli
